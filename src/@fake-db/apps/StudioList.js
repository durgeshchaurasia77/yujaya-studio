...
// GET ALL DATA
mock.onGet('/api/studio/list/all-data').reply(200, data.studio)

// POST: Add new studio
mock.onPost('/apps/studio/add-studio').reply(config => {
  // Get event from post data
  const studio = JSON.parse(config.data)

  const { length } = data.studio
  let lastIndex = 0
  if (length) {
    lastIndex = data.studio[length - 1].id
  }
  studio.id = lastIndex + 1

  data.studio.unshift(studio)

  return [201, { studio }]
})

// GET Updated DATA
mock.onGet('/api/studio/list/data').reply(config => {
  const { q = '', perPage = 10, page = 1, role = null, currentPlan = null, status = null } = config


  /* eslint-disable  */
  const queryLowered = q.toLowerCase()
  const filteredData = data.studio.filter(
    studio =>
      (studio.username.toLowerCase().includes(queryLowered) || studio.fullName.toLowerCase().includes(queryLowered)) &&
      studio.role === (role || studio.role) &&
      studio.currentPlan === (currentPlan || studio.currentPlan) &&
      studio.status === (status || studio.status)
  )
  /* eslint-enable  */
  ...
  return [
    200,
    {
      studio: paginateArray(filteredData, perPage, page),
...
// GET USER
mock.onGet('/api/studio/user').reply(config => {
  const { id } = config
  const studio = data.studio.find(i => i.id === id)
  return [200, { studio }]
})

// DELETE: Deletes User
mock.onDelete('/apps/studio/delete').reply(config => {
  // Get studio id from URL
  let studioId = config.id

  // Convert Id to number
  studioId = Number(studioId)

  const studioIndex = data.studio.findIndex(t => t.id === studioId)
  data.studio.splice(studioIndex, 1)
